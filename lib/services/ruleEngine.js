  ...
  if (fields.issue_date) {
    ...
    if (issueDate > today) {
      results.push({
        check: "issue_date",
        status: "fail",
        message: "Issue date is in the future — document may be fraudulent or incorrectly dated",
      });
      verdict = "fail";
    } else {
      results.push({
  ...
  if (fields.issue_date && fields.expiry_date) {
    ...
    if (expiryDate <= issueDate) {
      results.push({
        check: "date_sequence",
        status: "fail",
        message: "Expiry date is before or the same as issue date — document is invalid",
      });
      verdict = "fail";
    } else {
      results.push({
